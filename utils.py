...
def write_helmholtz_summary(model, model_input, gt, model_output, writer, total_steps, prefix='train_'):
    ...
    with torch.no_grad():
        ...
        min_max_summary(prefix + 'pred_abs', torch.sqrt(pred[..., 0::2, :, :] ** 2 + pred[..., 1::2, :, :] ** 2),
                        writer, total_steps)
...
def write_image_summary_small(image_resolution, mask, model, model_input, gt, model_output, writer, total_steps,
                              prefix='train_'):
    if mask is None:
...
def write_laplace_summary(model, model_input, gt, model_output, writer, total_steps, prefix='train_'):
    ...
    writer.add_image(prefix + 'comp_gt_vs_pred_laplace',
...
def write_gradients_summary(model, model_input, gt, model_output, writer, total_steps, prefix='train_'):
    ...
    gt_img = dataio.lin2img(gt['img'])
    pred_img = dataio.lin2img(model_output['model_out'])
    pred_img_up = dataio.lin2img(model_output['model_out_up'])
    ...
    pred_gradients_up = diff_operators.gradient(model_output['model_out_up'], model_output['model_in_up'])
    pred_grads_img_up = dataio.grads2img(dataio.lin2img(pred_gradients_up))

    output_vs_gt_gradients = torch.cat((gt_grads_img, pred_grads_img), dim=-1)
    writer.add_image(prefix + 'comp_gt_vs_pred_gradients',
    ...
    # Plot pred image upsample.
    pred_img_up_clamp = dataio.rescale_img(pred_img_up, mode='clamp')
    writer.add_image(prefix + 'pred_img_up', make_grid(pred_img_up_clamp, scale_each=False, normalize=False),
                     global_step=total_steps)

    # Plot pred gradient upsample.
    writer.add_image(prefix + 'pred_grad_up', make_grid(pred_grads_img_up, scale_each=False, normalize=True),
                     global_step=total_steps)
